...
from be.model.database import clearDatabaseSession, getDatabaseSession
from be.model.tables import NewOrder
...
class TestCancelOrder:
    ...
    def test_outdate_cancel(self):
        ...
        session.query(NewOrder).filter(NewOrder.order_id == order_id).update(
            {"order_time": curDateTime - timedelta(minutes=15)})
        session.commit()
        session.close()
