...
from be.model.tables import NewOrder, NewOrderDetail, StoreBook, UserStore, book_id_exist, store_id_exist, user_id_exist
...
class Seller:
    ...
    def add_stock_level(self, user_id: str, store_id: str, book_id: str, add_stock_level: int):
        try:
            ...
            with getDatabaseSession() as session:
                book = session.query(StoreBook).filter_by(store_id=store_id, book_id=book_id).first()
    ...
    def deliver_order(self, order_id: str):
        try:
            order_session = getDatabaseSession()
            order = order_session.query(NewOrder).filter_by(order_id=order_id).first()
            if not order:
            ...
            if not store_id_exist(store_id):
                return error.error_non_exist_store_id(store_id)
            with getDatabaseSession() as session:
                detailed_order = session.query(NewOrderDetail).filter_by(order_id=order_id).all()
            if not detailed_order:
                return error.error_invalid_order_id(order_id)

            for detail in detailed_order:
                book_id = detail.book_id
                count = detail.count
                if not book_id_exist(store_id, book_id):
                    return error.error_non_exist_book_id(book_id)

                book_session = getDatabaseSession()
                book = book_session.query(StoreBook).filter_by(store_id=store_id, book_id=book_id).first()
